...
def get_procedure_name(p: str) -> Tuple[str, str]:
    ...
    schema, name = parse_object_name(name)
    return schema, name


def parse_object_name(name: str, default_schema: str=DEFAULT_SCHEMA) -> Tuple[str, str]:
    ...
    :param default_schema: default schema, defaults to config file definition
    :return: tuple (schema_string, name_string)
    """
...
def parse_statement(dmltype: str, s: str, mapper: Mapper) -> Query:
...
def get_updated_columns(tree: ParserRuleContext) -> List[str]:
    ...
    for c in tree.getChildren():
        ...
        if isinstance(c, MySqlParser.UpdatedElementContext):
            ...
            cols.extend(get_updated_columns(c))

    return cols


def get_inserted_columns(tree: ParserRuleContext) -> List[str]:
...
def parse_update(tree: ParserRuleContext, dmltype: str) \
        -> Optional[Query]:
    ...
    q.target_table = get_target_table(tree)
    q.join_table = get_source_tables_update(tree)
    q.target_columns = get_updated_columns(tree)
...
def get_inserted_tables(tree: ParserRuleContext) \
    ...
    for child in tree.getChildren():
        if isinstance(child, MySqlParser.InsertStatementValueContext):
            from_table = get_source_tables_insert(child, 'from')
            join_table = get_source_tables_insert(child, 'join')
...
def parse_insert(tree: ParserRuleContext, dmltype: str) -> Query:
    ...
    q = Query(operation=dmltype)
    q.target_table = get_target_table(tree)
    q.from_table, q.join_table = get_inserted_tables(tree)
    q.target_columns = get_inserted_columns(tree)

    return q


def get_delete_table(tree: ParserRuleContext) \
        -> Optional[List[Table]]:
    ...
    for child in tree.getChildren():
        if isinstance(child, MySqlParser.DeleteStatementValueContext):
            return get_source_tables_insert(child, 'from')


def parse_delete(tree: ParserRuleContext, dmltype: str) -> Query:
...
def get_source_tables_update(tree: ParserRuleContext) \
        -> List[Table]:
    ...
    for c in tree.getChildren():
        if isinstance(c, MySqlParser.JoinPartContext):
            tables.extend(get_all_tables(c))
        elif not (isinstance(c, TerminalNode) or isinstance(c, ErrorNode)):
            tables.extend(get_source_tables_update(c))

    return tables


def get_source_tables_insert(tree: ParserRuleContext, clause: str) \
        -> List[Table]:
    ...
    for c in tree.getChildren():
        if isinstance(c, MySqlParser.FromClauseContext):
            ...
            for i in froms.tableSource():
                if clause == 'from':
                    f = i.tableSourceItem()
                    tables.extend(get_all_tables(f))
                else:
                    f = i.joinPart()
                    for x in f:
                        tables.extend(get_all_tables(x))
        elif not (isinstance(c, TerminalNode) or isinstance(c, ErrorNode)):
            tables.extend(get_source_tables_insert(c, clause))
    return tables


def get_target_table(tree: ParserRuleContext) -> Table:
    ...
    for c in tree.getChildren():
        ...
        if isinstance(c, MySqlParser.TableNameContext):
            schema, name = parse_object_name(c.getText().lower())
            t = Table(name, schema)
            return t
        elif not (isinstance(c, TerminalNode) or isinstance(c, ErrorNode)):
            return get_target_table(c)


def get_all_tables(tree: ParserRuleContext) -> List[Table]:
